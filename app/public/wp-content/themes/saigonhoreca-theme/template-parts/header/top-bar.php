        ...
        <div class="sh-topbar__marquee-wrap">
            ...
            if (isset($args['marquee_lines']) && is_array($args['marquee_lines'])) {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($text === false) {
                        $defaults = [
                            1 => 'Chào mừng kỷ niệm 15 năm thành lập Saigon Horeca!',
            ...
            <?php endforeach; ?>
                <span class="sh-topbar__item"><?php echo function_exists('sh_icon') ? sh_icon('phone', 'sh-topbar__item-icon') : ''; ?> <?php esc_html_e('Hotline:', 'saigonhoreca'); ?> <?php echo esc_html($contact['hotline'] ?? ''); ?></span>
...
<input type="search" class="sh-topbar__search-input" placeholder="<?php esc_attr_e('Tìm kiếm...', 'saigonhoreca'); ?>" value="<?php echo get_search_query(); ?>" name="s" />
                  <button type="submit" class="sh-topbar__search-btn" aria-label="<?php esc_attr_e('Tìm kiếm', 'saigonhoreca'); ?>">
                     <?php echo sh_icon('search', 'sh-topbar__search-icon'); ?>
                 </button>
             </form>
<a href="<?php echo home_url('/lien-he'); ?>" class="sh-topbar__cta">
                  <span><?php esc_html_e('LIÊN HỆ NGAY', 'saigonhoreca'); ?></span>
