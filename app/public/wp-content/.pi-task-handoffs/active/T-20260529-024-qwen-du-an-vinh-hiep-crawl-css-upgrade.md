...
worktree_path: null
project_slug: du-an-vinh-hiep
project_abbr: vhiep
---

## 0. User Original Intent (Phase 0 — Verbatim)

> full cac project chua lam — upgrade CSS quality cho du-an-vinh-hiep len gold-standard ngang rokafella/casa-maria

Context: Batch upgrade theo master plan project/PILLAR-UPGRADE-PLAN.md.

# T-20260529-024 | qwen | du-an-vinh-hiep Crawl + CSS Upgrade
...
| project | Du an Vinh Hiep (du-an-vinh-hiep), abbr vhiep, ~13 sections |

## II. Goal
**Workflow**: PHASE 0 = crawl static-mirror (§G) → PHASE 1+ = nâng CSS. Nang CSS/layout/animation cua template-parts/project-pillar/du-an-vinh-hiep/ tu render PHANG len gold-standard. Giu noi dung text + class prefix. Chi nang VISUAL theo quality-bar.
...
## IV. Allowed Scope
- template-parts/project-pillar/du-an-vinh-hiep/**  (refactor CSS + markup)
- single-project/du-an-vinh-hiep.php + .css  (chi neu can class root/order)
- assets/css/imports/_imports-project.css  (CHI block du-an-vinh-hiep)

- static-mirror/saigonhoreca.vn/du-an-vinh-hiep/** + saigonhoreca.com/du-an-vinh-hiep/**  (PHASE 0 crawl output)

KHONG: project khac · shared _container/_caption/_cta/_related · _tokens.css (chi DUNG) · inc/core · functions.php

## V. Out Of Scope
- Doi noi dung text (da dung live) · doi class prefix pp-*-vhiep
...
0. **PHASE 0 — CRAWL static-mirror (BAT BUOC TRUOC CSS)** theo §G+§G.1a+§G.5 — ⚠️ CHỈ .vn (vn-only, KHÔNG có .com): .vn <- local /du-an/du-an-vinh-hiep/ ; .com = KHONG CO (vn-only — chi crawl .vn). Anh PROJECT-ONLY (loc content-region + token), strip header/footer/related, localize css/. -> static-mirror/<domain>/du-an-vinh-hiep/{index.html,images/,css/}
...
- grep hardcode hex: template-parts/project-pillar/du-an-vinh-hiep/*.css (phai 0 brand hex)
...
- PHASE 0 mirror: find static-mirror/saigonhoreca.vn/du-an-vinh-hiep/images -type f | wc -l (>0) ; (vn-only: bo qua .com)
- npm run build:project
- curl -k -I https://saigonhoreca.local/du-an/du-an-vinh-hiep/
- git status --short
...
- [ ] Class prefix toan pp-*-vhiep + BEM
...
- [ ] PHASE 0 .com: — N/A (vn-only — khong co trang .com)
- [ ] Scope: chi du-an-vinh-hiep files

## IX. Copy-Paste Prompt cho Qwen
You are Qwen3.7-Max (Tier 2, Luna-Proxy). Execute T-20260529-024: upgrade CSS quality cho "Du an Vinh Hiep" (du-an-vinh-hiep) len gold-standard.
...
TARGET: saigonhoreca-theme | SCOPE: template-parts/project-pillar/du-an-vinh-hiep/** + single-project/du-an-vinh-hiep.php + _imports-project.css(block du-an-vinh-hiep)
PHASE 0 (BAT BUOC TRUOC CSS) — crawl static-mirror theo §G+§G.5 — ⚠️ CHỈ .vn (vn-only): .vn <- local https://saigonhoreca.local/du-an/du-an-vinh-hiep/ ; .com vn-only (bo qua). Anh PROJECT-ONLY (loc content-region §G + token), strip header/footer/related, localize css/. ⚠️§G.6 (BAT BUOC dung): tai anh TU PRODUCTION (saigonhoreca.com hoac saigonhoreca.vn — KHONG .local vi local 404 ra HTML), VERIFY moi anh LA IMAGE THAT (`file -b <f> | grep -i image`, XOA file 404-HTML doi lot .jpg), localize HET URL tuyet doi (0 con lai), images PHAI >0 (=0 hoac co fake-HTML -> REJECTED). ROI MOI:
DO: refactor CSS -> token vars (var(--p)/--bc/color-mix); >=6/9 techniques; wrap pp-container-shared; anh boc pp-image-caption-shared; giu text + prefix pp-*-vhiep; prefers-reduced-motion.
DONT: doi text/prefix, touch project khac/shared/tokens, hardcode hex brand.
VERIFY (paste raw): token compliance + shared partials + technique count + build + route + git status. Khi XONG: tao file returns/T-20260529-024-return.md theo system/templates/RETURN-REPORT.md (scaffold: bash system/scripts/new-return.sh T-20260529-024) — dien frontmatter (status/scope_clean/gates_passed/recommendation) + paste RAW evidence. File .md NAY la deliverable, KHONG chi dan chat. Luna loi -> STOP state rejected.
...
| Token/Prefix | pending | 0 hex brand, prefix pp-*-vhiep |
| Techniques | pending | >=6/9 |
| Scope | pending | chi du-an-vinh-hiep |
