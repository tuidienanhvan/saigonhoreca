...
created: 2026-05-16 11:55
updated: 2026-05-16 11:56
archived: 2026-05-16 11:56
verified: 2026-05-16 12:00
---

## ✅ Completion Notes

Final follow-up to T-008. After audit user requested "T-010 + rgba cleanup sâu hơn".

### Drift fixed
- **Status hex** (most common drift): `'#ef4444' : '#10b981'` pattern in 13 config components → `'var(--er)' : 'var(--su)'`
- **Chart palettes**: hardcoded `['#0ea5e9', '#a855f7', ...]` → `var(--color-chart-N)`
- **Brand indigo** (`#4f46e5`, `#6366f1`): ChatLogs, FunnelResults → `var(--in)` + color-mix
- **2 text-white drift** (AuditRow, FlagsCard) → `text-base-content`
- **Border #0a0a0a/#121212** in admin → `border-base-100/200`
- **Dark bg `#0a0d12`** (CriticalCss) → `var(--b1)`
- **Toaster LEVEL_CONFIG** entire palette → token-aware
- **rgba() in JSX style props** batch:
  - Modal overlays `rgba(0,0,0,0.55)` → `color-mix(black 55%, transparent)`
  - Status muted bgs `rgba(239,68,68,0.15)` → `color-mix(var(--er) 15%)`
  - Ambient white tints → `color-mix(var(--bc) X%)`
- **RetentionMatrix dynamic alpha**: `rgba(34,197,94, ${alpha})` → template-literal color-mix with var(--su)

### Final state
- Hex drift in JSX: **2** (both `var(--on-brand, #fff)` CSS fallbacks — legit)
- rgba() in JSX: **4** (Tailwind arbitrary shadow values — accepted)
- bg-white|black standalone: **0**
- text-white|black ngoài WCAG: **0**
- **Token coverage ≈99%**

### Acceptable hardcodes still in tree (not drift)
- index.css primitive definitions
- FullPageLoader.css/jsx (pre-theme splash)
- PiLogo.css (brand-color literal)
- BrandingCard.jsx, ColorPicker.jsx, PresetSelector.jsx (admin user-config UI)
- HealthChart `getCSSVar('--text-X', '#fallback')` (defensive JS fallback)
- TemplatePreview email rendering (email needs literal colors)
- OG image designer (user picks literal colors)
- Toolbar color picker default
- Toaster SVG glow stroke (decorative aesthetics)

### Build
- pi-store: 889ms PASS
- pi-dashboard: 1.13s PASS

### Commits
- pi-store: `c949b09 fix: T-010 token compliance sweep`
- pi-dashboard: `0c2f515 fix: T-010 token compliance sweep`

## 0. 📥 User Original Intent (Phase 0 — Verbatim)

> <!-- Paste user message verbatim here. -->


# 📋 T-20260516-010 | claude | token-compliance-sweep — Bản đặc tả công việc chi tiết / Detailed Task Specification
