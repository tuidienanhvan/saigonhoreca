...
updated: 2026-06-01 21:07
archived: 2026-06-01 21:07
archived: null
...
worktree_path: null
project_slug: yuzu-omakase
project_abbr: yuzu
---

## 0. User Original Intent (Phase 0 — Verbatim)

> full cac project chua lam — upgrade CSS quality cho yuzu-omakase len gold-standard ngang rokafella/casa-maria

Context: Batch upgrade theo master plan project/PILLAR-UPGRADE-PLAN.md.

# T-20260529-009 | qwen | yuzu-omakase Crawl + CSS Upgrade
...
| project | Yuzu Omakase (yuzu-omakase), abbr yuzu, ~8 sections |

## II. Goal
**Workflow**: PHASE 0 = crawl static-mirror (§G) → PHASE 1+ = nâng CSS. Nang CSS/layout/animation cua template-parts/project-pillar/yuzu-omakase/ tu render PHANG len gold-standard. Giu noi dung text + class prefix. Chi nang VISUAL theo quality-bar.
...
## IV. Allowed Scope
- template-parts/project-pillar/yuzu-omakase/**  (refactor CSS + markup)
- single-project/yuzu-omakase.php + .css  (chi neu can class root/order)
- assets/css/imports/_imports-project.css  (CHI block yuzu-omakase)

- static-mirror/saigonhoreca.vn/yuzu-omakase/** + saigonhoreca.com/yuzu-omakase/**  (PHASE 0 crawl output)

KHONG: project khac · shared _container/_caption/_cta/_related · _tokens.css (chi DUNG) · inc/core · functions.php

## V. Out Of Scope
- Doi noi dung text (da dung live) · doi class prefix pp-*-yuzu
...
0. **PHASE 0 — CRAWL static-mirror (BAT BUOC TRUOC CSS)** theo §G+§G.1a+§G.5 — ⚠️ CRAWL CẢ 2 MIRROR (.vn VÀ .com, KHÔNG bỏ cái nào): .vn <- local /du-an/yuzu-omakase/ ; .com <- LIVE https://saigonhoreca.com/yuzu-omakase/ (English, top-level, KHONG /project/). Anh PROJECT-ONLY (loc content-region + token), strip header/footer/related, localize css/. -> static-mirror/<domain>/yuzu-omakase/{index.html,images/,css/}
...
- grep hardcode hex: template-parts/project-pillar/yuzu-omakase/*.css (phai 0 brand hex)
...
- PHASE 0 mirror: find static-mirror/saigonhoreca.vn/yuzu-omakase/images -type f | wc -l (>0) ; grep lang= static-mirror/saigonhoreca.com/yuzu-omakase/index.html (phai en)
- npm run build:project
- curl -k -I https://saigonhoreca.local/du-an/yuzu-omakase/
- git status --short
...
- [ ] PHASE 0 .com: — crawled tu LIVE /yuzu-omakase/ (lang=en, anh project-only, 0 broken)
- [ ] Scope: chi yuzu-omakase files

## IX. Copy-Paste Prompt cho Qwen
You are Qwen3.7-Max (Tier 2, Luna-Proxy). Execute T-20260529-009: upgrade CSS quality cho "Yuzu Omakase" (yuzu-omakase) len gold-standard.
...
TARGET: saigonhoreca-theme | SCOPE: template-parts/project-pillar/yuzu-omakase/** + single-project/yuzu-omakase.php + _imports-project.css(block yuzu-omakase)
PHASE 0 (BAT BUOC TRUOC CSS) — crawl static-mirror theo §G+§G.5 — ⚠️ CRAWL ĐỦ CẢ 2 (.vn VÀ .com): .vn <- local https://saigonhoreca.local/du-an/yuzu-omakase/ ; .com <- LIVE https://saigonhoreca.com/yuzu-omakase/ (English). Anh PROJECT-ONLY (loc content-region §G + token), strip header/footer/related, localize css/. ⚠️§G.6 (BAT BUOC dung): tai anh TU PRODUCTION (saigonhoreca.com hoac saigonhoreca.vn — KHONG .local vi local 404 ra HTML), VERIFY moi anh LA IMAGE THAT (`file -b <f> | grep -i image`, XOA file 404-HTML doi lot .jpg), localize HET URL tuyet doi (0 con lai), images PHAI >0 (=0 hoac co fake-HTML -> REJECTED). ROI MOI:
DO: refactor CSS -> token vars (var(--p)/--bc/color-mix); >=6/9 techniques; wrap pp-container-shared; anh boc pp-image-caption-shared; giu text + prefix pp-*-yuzu; prefers-reduced-motion.
DONT: doi text/prefix, touch project khac/shared/tokens, hardcode hex brand.
VERIFY (paste raw): token compliance + shared partials + technique count + build + route + git status. Khi XONG: tao file returns/T-20260529-009-return.md theo system/templates/RETURN-REPORT.md (scaffold: bash system/scripts/new-return.sh T-20260529-009) — dien frontmatter (status/scope_clean/gates_passed/recommendation) + paste RAW evidence. File .md NAY la deliverable, KHONG chi dan chat. Luna loi -> STOP state rejected.
...
| Token/Prefix | pending | 0 hex brand, prefix pp-*-yuzu |
| Techniques | pending | >=6/9 |
| Scope | pending | chi yuzu-omakase |
...
- **2026-06-01 21:06**: State dispatched → returned
- **2026-06-01 21:06**: State returned → verified
